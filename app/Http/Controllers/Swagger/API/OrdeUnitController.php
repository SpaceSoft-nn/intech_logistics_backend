 ...
 *
 *      path="/api/orders/{OrderUnit::uuid}",
 *      summary="Обновить частично данные заказа.",
 ...
 *      summary="Получить всех подрятчиков откликнувшиеся на заказ (Возвращаем в зависимости от типа организации)",
 *      description="Получить всех подрятчиков которые откликнулись на заказ, и заинтересованы в работе, возвращает: organization_order_unit_invoces",
 *      tags={"Order Unit"},
 *
 *      @OA\Response(
 ...
 *
 * @OA\Post(
 *     path="api/orders/{OrderUnit::uuid}/agreements",
 *     tags={"Order Unit"},
 ...
 *          description="Successfully added a contractor to the order.",
 *          @OA\JsonContent(
 *              @OA\Property(property="data", ref="#/components/schemas/AgreementOrderResource"),
 ...
 *     path="api/orders/{OrderUnit::uuid}/agreements",
 *     tags={"Order Unit"},
 *     summary="Возвращаем выбранный отклик Заказчиком",
 ...
 * @OA\patch(
 *
 *     path="api/orders/agreements/{agreementOrderAccept::uuid}/accept",
 *     tags={"Order Unit"},
 *     summary="ЭДО Утверждения Двух-стороннего договор, о принятии в работу Заказа, со стороны Заказчика/Подрядчика",
 ...
 *          description="Заказчик успешно выбрал подрятчика, запись создана.",
 *          @OA\JsonContent(
 *              @OA\Property( property="data", type="array", @OA\Items(ref="#/components/schemas/AgreementOrderAcceptResource") ),
*               @OA\Property(property="message0", type="string", example="Записи успешна возвращены."),
 ...
 *     path="api/orders/agreements/{agreementOrderAccept::uuid}/accept",
 *     tags={"Order Unit"},
 *     summary="//вернуть agreementOrderAccept по uuid",
 *     description="Заказчик/Подрядчик - true/true - что бы была возможность создать Transfer",
 *     @OA\Parameter(
 *          name="agreementOrderAccept::uuid",
 *          in="path",
 ...
 *  @OA\Patch(
 *
 *      path="/api/orders/{OrderUnit::uuid}/draft",
 *      summary="Обновить данные заказа при статусе 'Черновик'.",
 *      description="Обновляет информацию о заказе по UUID при статусе заказа Черновик",
 *      tags={"Order Unit"},
 *      @OA\Parameter(
 *          name="OrderUnit::uuid",
 *          in="path",
 ...
 *                                ),
 *                            ),
 *                            @OA\Property(property="type_transport_weight", type="string", description="**Тип транспортировки по габаритам. Обязательное поле.** extraSmall: до 0.8 тонны, small: до 1.5 тонн , medium: до 3 тонн, large: до 5 тонн, extraLarge: до 10 тонн, superSize: Более 10 тонн", enum={"extraSmall", "small", "medium", "large", "extraLarge", "superSize"}, example="small"),
 *                            @OA\Property(property="organization_id", type="string", format="uuid", description="**UUID организации. Поле обязательно и должно существовать в таблице organizations.**", example="123e4567-e89b-12d3-a456-426614174000"),
 *                            @OA\Property(property="end_date_order", type="string", format="date", description="**Дата окончания заказа. Обязательное поле.**", example="2023-10-15"),
 *                            @OA\Property(property="type_load_truck", type="string", description="**Тип загрузки грузовика. Обязательное поле. Возможные значения: ftl, ltl, custom.**", enum={"ftl", "ltl", "custom"}, example="ftl"),
 *                            @OA\Property(property="order_total", type="number", format="float", description="**Цена заказа. Обязательное поле.**", example=50000.00),
 ...
 *           @OA\JsonContent(
 *               @OA\Property(property="data", ref="#/components/schemas/OrderUnitResource"),
 *               @OA\Property(property="message", type="string", example="Update Order Unit Successfully."),
 *           ),
 *       ),
 *
 *       @OA\Response(
