...
//Repository Logic Test
    ...
    */
    ...
    /**
     * Проверка отправки тестового сообщение
     * @return void
     */
    public function test_it_sends_email_notification()
    {
        // Подготовка: Подменяем отправку почты
        Mail::fake();

        // Здесь вызывайте ваш код, отправляющий почту
        Mail::to('test@example.com')->send(new SendMessageSmtpNotification(1));

        // Проверка: убедитесь, что письмо было отправлено
        Mail::assertQueued(SendMessageSmtpNotification::class, function ($mail) {
            return $mail->hasTo('test@example.com') && $mail->data === 1;
        });
    }
